...
describe("ConfigManager", () => {
  ...
  it("writes providers.json with 0600 permissions", () => {
    const config = new ConfigManager();
    config.setApiKey("openai", "sk-secret");
    const mode = statSync(paths.providers()).mode & 0o777;
    expect(mode).toBe(0o600);
    const raw = JSON.parse(readFileSync(paths.providers(), "utf8")) as Record<string, { apiKey: string }>;
    expect(raw.openai?.apiKey).toBe("sk-secret");
  });
